...
## label.lab
This file contains the population of the individual contained in the vcf file. 

## medeas_to_plink.py
This script converts Medeas internal format back to PLINK transposed text format (`.tped/.tfam`) or PLINK binary format (`.bed/.bim/.fam`). It expects:
- a Medeas genotype file (`-sf`)
- a Medeas haploid label file (`-lf`)
- an output prefix (`-o`)

Example:
```bash
python medeas_to_plink.py -sf ../test/snp.dat -lf ../test/pop_label.dat -o out_prefix
python medeas_to_plink.py -sf ../test/snp.dat -lf ../test/pop_label.dat -o out_prefix --make-bed
```